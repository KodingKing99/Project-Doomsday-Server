...
public sealed class InMemoryFileRepository : IFileRepository
{
    ...
    public Task<IReadOnlyList<FileRecord>> ListAsync(int skip, int take, CancellationToken cancellationToken)
    {
    ...
    public Task UpsertAsync(FileRecord file, CancellationToken cancellationToken) { _db[file.Id] = file; return Task.CompletedTask; }
    public Task DeleteAsync(Guid id, CancellationToken cancellationToken) { _db.TryRemove(id, out _); return Task.CompletedTask; }
}
